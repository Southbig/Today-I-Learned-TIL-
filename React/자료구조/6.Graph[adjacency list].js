...
class GraphWithAdjacencyList {
  ...
  removeEdge(fromVertex, toVertex) {
    ...
    if (this.hasEdge(fromVertex, toVertex)) {
      const toindex = this.vertices[fromVertex].indexOf(toVertex);
      this.vertices[fromVertex].splice(toindex, 1);
    }
    // TODO:  두번째 정점의 인접 리스트에 첫번째 정점이 있을 경우
    if (this.hasEdge(toVertex, fromVertex)) {
      const fromindex = this.vertices[toVertex].indexOf(fromVertex);
      this.vertices[toVertex].splice(fromindex, 1);
    }
  }
